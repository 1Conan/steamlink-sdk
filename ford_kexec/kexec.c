...
static int __init kexec_module_init(void)
{
  ...
  */
	...
	syscall_table=(void **)aquire_sys_call_table();
	if(syscall_table==NULL) {
		printk(KERN_ERR "Cannot find the system call address\n"); 
		return -1;  // do not load
	}
...
int kernel_kexec(void)
{
...
#endif
	{
    //unsigned long (*sym_kallsyms_lookup_name)(const char *name) = (void *)kallsyms_lookup_name_ptr;
    unsigned long (*sym_kallsyms_lookup_name)(const char *name) = (void *)kallsyms_lookup_name;
    void (*sym_kernel_restart_prepare)(char *cmd) = (void *)sym_kallsyms_lookup_name("kernel_restart_prepare");
    int (*sym_disable_nonboot_cpus)(void) = (void *)sym_kallsyms_lookup_name("disable_nonboot_cpus");
    int (*sym_platform_cpu_disable)(unsigned int) = (void *)sym_kallsyms_lookup_name("platform_cpu_disable");
    int (*sym_cpu_down)(unsigned int) = (void *)sym_kallsyms_lookup_name("cpu_down");
    ...
    void (*sym_cpu_hotplug_enable)(void) = (void *)sym_kallsyms_lookup_name("cpu_hotplug_enable");
		...
		sym_kernel_restart_prepare(NULL);
		printk(KERN_EMERG "Machine shutdown\n");
    kernel_restart_prepare(NULL);
	}

  printk(KERN_EMERG "Kexec whatever\n");
	machine_kexec(kexec_image);

#ifdef CONFIG_KEXEC_JUMP
  printk(KERN_EMERG "Jump is defined\n");
	if (kexec_image->preserve_context) {
    printk(KERN_EMERG "Preserving context\n");
		syscore_resume();
 Enable_irqs:
		local_irq_enable();
 Enable_cpus:
 ...
 Unlock:
  printk(KERN_EMERG "Unlocking mutex\n");
	mutex_unlock(&kexec_mutex);
	return error;
}
